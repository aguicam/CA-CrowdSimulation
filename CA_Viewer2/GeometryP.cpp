...
float Plane::distPoint2Plane(const glm::vec3& point){
	float dist;
    return dist = glm::dot(point, normal) + dconst;//-colRadi;//Col radi añadido para colison
}
...
bool Plane::inRangeOfWall(const glm::vec3& point){
    if(isZrange){
        if(point.z>wallMinZ && wallMaxZ<point.z){return true;}
        else{return false;}
    }else{
        if(point.x>wallMinX && wallMaxX<point.x){return true;}
        else{return false;}
    }

}

void Plane::setColRadi(float radi){
    colRadi=radi;
    std::cout<<"Radi colision set"<<std::endl;
}
